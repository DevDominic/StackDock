      ...
      <div className="modal settings-modal" onMouseDown={(event) => event.stopPropagation()}>
        ...
        {tab === 'terminal' ? (
          <div className="settings-tab-body">
            ...
            <label><input type="checkbox" checked={draft.terminal.persistentSessionCache !== false} onChange={(event) => setDraft({ ...draft, terminal: { ...draft.terminal, persistentSessionCache: event.target.checked } })} /> Keep inactive terminal sessions mounted</label>
            <p className="muted config-hint">Speeds up switching back to previous terminal sessions by hiding them instead of remounting/replaying them. Uses more memory/GPU resources.</p>
            <h3>Smart Input</h3>
            <label className="settings-toggle-row"><span><b>Enable Smart Input bar</b><span className="muted code-font-note">Show a multi-line command composer beneath each terminal.</span></span><input type="checkbox" checked={draft.terminal.smartInput?.enabled === true} onChange={(event) => setDraft({ ...draft, terminal: { ...draft.terminal, smartInput: { enabled: event.target.checked, enterToSend: draft.terminal.smartInput?.enterToSend !== false, sendEnter: draft.terminal.smartInput?.sendEnter === true } } })} /></label>
            <label className="settings-toggle-row"><span><b>Enter sends command</b><span className="muted code-font-note">Use Shift+Enter for new lines when enabled.</span></span><input type="checkbox" checked={draft.terminal.smartInput?.enterToSend !== false} onChange={(event) => setDraft({ ...draft, terminal: { ...draft.terminal, smartInput: { enabled: draft.terminal.smartInput?.enabled === true, enterToSend: event.target.checked, sendEnter: draft.terminal.smartInput?.sendEnter === true } } })} /></label>
            <label className="settings-toggle-row"><span><b>Execute after sending</b><span className="muted code-font-note">Send a terminal Enter after Smart Input writes the command.</span></span><input type="checkbox" checked={draft.terminal.smartInput?.sendEnter === true} onChange={(event) => setDraft({ ...draft, terminal: { ...draft.terminal, smartInput: { enabled: draft.terminal.smartInput?.enabled === true, enterToSend: draft.terminal.smartInput?.enterToSend !== false, sendEnter: event.target.checked } } })} /></label>
