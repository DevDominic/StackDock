...
interface CreateInput { workspaceId: string; workspaceName: string; workspacePath: string; profileId: string; cwd?: string; name?: string; startupCommand?: string; }
...
export const useSessionStore = create<SessionState>((set, get) => ({
  ...
  async createSession(input) {
    const terminal = await api.terminal.create(input.profileId, input.cwd ?? input.workspacePath, input.name, input.startupCommand);
  ...
  replaceSession(id, next) { set({ sessions: get().sessions.map((session) => session.id === id ? next : session), activeSessionId: next.id, activeWorkspaceId: next.workspaceId }); },
  setActiveSession(id) { const session = get().sessions.find((item) => item.id === id); set({ activeSessionId: id, activeWorkspaceId: session?.workspaceId ?? get().activeWorkspaceId }); },
