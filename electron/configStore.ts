...
export async function loadSettings(): Promise<StackDockSettings> {
  ...
  try {
    ...
    return { ...defaults, ...raw, editor: { ...defaults.editor, ...raw.editor }, terminal: { ...defaults.terminal, ...raw.terminal }, terminalProfiles: raw.terminalProfiles?.length ? raw.terminalProfiles : defaults.terminalProfiles };
  } catch {
    return defaults;
  }
}
